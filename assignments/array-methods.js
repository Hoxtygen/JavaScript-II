...
runners.filter(large => {
    large.shirt_size === "L" ? largeShirts.push(large) : "";
})
...
ticketPriceTotal = runners.reduce((sum, acc) => sum + acc.donation, 0)
...
let runnersEmail = [];
    runners.filter(runner => {
       return runnersEmail.push(runner.email)
    });
    console.log(runnersEmail)
    //console.table(runnersEmail)
...
runners.forEach(runner => {
    if (runner.shirt_size == "XS") {
        runner.message = "Due to production reason, we could not get the appropriate size for you right now, please bear with us as we sort things out";
    }
});
console.log(runners);
